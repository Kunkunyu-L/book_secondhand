...
const detailImgsUploading = ref(false)
...
const openEdit = (row: any) => {
  ...
  detailImgList.value = row.detail_imgs ? row.detail_imgs.split(',').filter((url: string) => url.trim()) : []
  dialogVisible.value = true
}
...
const handleDetailImgUpload = async (options: { file: File }) => {
  detailImgsUploading.value = true
  try {
    const { url } = await uploadImage(options.file, 'platform')
    detailImgList.value.push(url)
    form.detail_imgs = detailImgList.value.join(',')
    ElMessage.success('详情图上传成功')
  } catch (e: any) {
    ElMessage.error(e.message || '上传失败')
  } finally {
    detailImgsUploading.value = false
  }
}
...
const handleExport = () => {
  exportToCSV(tableData.value, [
    ...
    { key: 'author', title: '作者' }, { key: 'publisher', title: '出版社' },
    { key: 'category_name', title: '分类' }, { key: 'original_price', title: '原价' },
    { key: 'price', title: '售价' }, { key: 'stock', title: '库存' }, { key: 'sales_count', title: '销量' },
    { key: 'status', title: '状态' }, { key: 'created_at', title: '创建时间' },
  ], '平台图书列表')
}
  ...
  <el-card shadow="never">
    ...
    <el-table :data="tableData" v-loading="loading" border stripe>
      ...
      <el-table-column label="原价" width="80" align="center">
        <template #default="{ row }">{{ row.original_price ? Number(row.original_price).toFixed(2) : '-' }}</template>
      </el-table-column>
      ...
      <el-table-column prop="sales_count" label="销量" width="70" align="center" />
      ...
      <el-table-column label="操作" width="160" fixed="right" align="center">
        <template #default="{ row }">
          <div style="white-space: nowrap;">
            <el-button type="primary" text size="small" @click="openEdit(row)" style="margin:0 2px">编辑</el-button>
            <el-button type="danger" text size="small" @click="handleDelete(row)" style="margin:0 2px">删除</el-button>
          </div>
        </template>
      </el-table-column>
    </el-table>
      ...
      <el-form :model="form" label-width="90px" style="max-height:65vh;overflow-y:auto;padding-right:12px">
        ...
        <el-form-item label="详情图">
          <el-upload
            ...
            multiple
          >
            <template #tip><span class="el-upload__tip">支持 jpg/png/gif/webp，单张不超过 5MB，可上传多张</span></template>
            <el-button type="primary" :loading="detailImgsUploading" :icon="Plus">上传详情图</el-button>
          </el-upload>
          <div v-if="detailImgList.length > 0" class="detail-imgs-list">
            <div v-for="(img, index) in detailImgList" :key="index" class="detail-img-item">
              <el-image :src="getImageUrl(img)" style="width:80px;height:100px;border-radius:4px" fit="cover" :preview-src-list="detailImgList.map(getImageUrl)" :initial-index="index" preview-teleported />
              <el-button type="danger" text size="small" @click="removeDetailImg(index)" style="margin-top:4px">删除</el-button>
            </div>
          </div>
        </el-form-item>
...
.detail-imgs-list { display: flex; gap: 12px; margin-top: 12px; flex-wrap: wrap; }
.detail-img-item { display: flex; flex-direction: column; align-items: center; }
</style>
