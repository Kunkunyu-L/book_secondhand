...
- ✅ 书市浏览：搜索、筛选、平台/用户图书混合展示、智能推荐
- ✅ 订单系统：直接购买、支付、物流、确认收货
...
- ✅ **数据看板**：运营数据统计与可视化（含地区统计）
- ✅ **书籍管理**：分类、平台图书、用户图书、审核
- ✅ **订单管理**：订单处理、退款售后、数据统计
- ✅ **用户管理**：用户列表、违规处理
- ✅ **交易管理**：支付记录、提现审核、优惠券
- ✅ **内容管理**：公告、轮播图、帮助中心、社区内容
- ✅ **客服管理**：会话监控、工单处理、话术库、FAQ（按分配客服控制可见范围）
- ✅ **系统配置**：角色权限、全局设置、通知模板
...
- ✅ 本项目已移除「购物车」「地区接口」「投诉举报」等功能，以当前代码和数据库脚本为准
- 📖 数据库表结构定义请参考 `secondhand.sql` 文件
- 🔧 新增模块时建议同步更新本文档
